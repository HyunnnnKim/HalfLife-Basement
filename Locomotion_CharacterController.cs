...
public class Locomotion_CharacterController : LocomotionProvider
{
    ...
    void Update()
    {
        ...
        {
            playerinput.SendHapticImpulse(VRDviceNode.LeftHand, 0.2f, 0.1f);
        }
        if (playerinput.GetRightHandInputData().otherButtonStatus.secondaryButton)
        {
            playerinput.SendHapticImpulse(VRDviceNode.LeftHand, 0.6f, 10f);
        }
    ...
    private void UpdateMove(Vector2 position, bool IsCliked)
    {
        ...
        switch (moveDirectionType)
        {
            ...
            case MoveDirectionType.HeadAndController:
                ...
                Vector3 headLookAtMidOfH = Quaternion.LookRotation(middleOfHands - head.transform.position).eulerAngles;
