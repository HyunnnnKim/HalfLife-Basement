...
namespace HalfLight.Input
{
    public class ControllerInput : Singleton<ControllerInput>
    {
        #region Devices
        [Header("Controllers")]
        [SerializeField] private XRController leftXRController = null;
        public XRController getLeftXRController { get { return leftXRController; } }
        [SerializeField] private XRController rightXRController = null;
        public XRController getRightXRController { get { return rightXRController; } }
        ...
        private void Update()
        {
            if (leftXRController.enableInputActions || rightXRController.enableInputActions)
            {
                ReadInput(leftXRController.inputDevice, ref leftHand);
                ReadInput(rightXRController.inputDevice, ref rightHand);

                CheckPrimary2DState(ref leftHand);
            }
        }
        #endregion
        ...
        private void CheckPrimary2DState(ref InputValues hand)
        {
            if (hand.primary2DValue.x > deadZone || hand.primary2DValue.x < -deadZone
                    || hand.primary2DValue.y > deadZone || hand.primary2DValue.y < -deadZone)
            {
                hand.primary2DValueState = true;
            }
            else
            {
                hand.primary2DValueState = false;
            }
        }
        #endregion
    }
